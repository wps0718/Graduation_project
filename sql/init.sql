...
CREATE TABLE IF NOT EXISTS `trade_order` (
  ...
  `remark` varchar(255) DEFAULT NULL COMMENT '买家备注（面交时间、协商说明等）',
  `status` tinyint(4) NOT NULL DEFAULT 1 COMMENT '订单状态 1-待接单 2-待面交 3-已完成 4-已评价 5-已取消',
  `seller_confirmed` tinyint(1) NOT NULL DEFAULT 0 COMMENT '卖家是否已确认交付 0-否 1-是',
  `buyer_confirmed` tinyint(1) NOT NULL DEFAULT 0 COMMENT '买家是否已确认交易 0-否 1-是',
...
CREATE TABLE IF NOT EXISTS `report` (
  ...
  `target_title` varchar(255) DEFAULT NULL COMMENT '被举报目标快照标题（商品标题/用户昵称）',
  `target_cover_image` varchar(255) DEFAULT NULL COMMENT '被举报目标快照封面（商品首图/用户头像）',
...
-- -----------------------------------------------------
-- 23. 代拿快递订单表（pickup_order）
-- 来源: sql/update/2026-05-30_pickup.sql
-- -----------------------------------------------------
...
-- -----------------------------------------------------
-- 25. 纠纷证据表（pickup_dispute_evidence）
-- 来源: sql/update/2026-05-30_pickup.sql
-- -----------------------------------------------------
...
-- 来源: sql/update/2026-05-30_pickup.sql
-- -----------------------------------------------------
ALTER TABLE `user` ADD COLUMN IF NOT EXISTS `pickup_score` DECIMAL(3,1) DEFAULT 5.0 COMMENT '代拿信用分';

-- -----------------------------------------------------
-- 通知表补充：代拿订单分类
-- 来源: sql/update/2026-05-30_pickup.sql
-- -----------------------------------------------------
ALTER TABLE `notification` MODIFY COLUMN `type` tinyint(4) NOT NULL COMMENT '1-交易成功 2-新消息 3-审核通过 4-审核驳回 5-系统公告 6-被收藏 7-订单取消 8-认证通过 9-认证驳回 10-评价提醒 25-纠纷裁决完成';
ALTER TABLE `notification` MODIFY COLUMN `category` tinyint(4) NOT NULL DEFAULT 1 COMMENT '消息分类 1-交易 2-系统 3-代拿订单';
